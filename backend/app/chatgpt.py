...
ユーザから、形式や要約内容についての指示が最初に与えられるため、それに従って要約してください。
入力は、以下の形式で与えられます。

「<*要約を依頼するユーザのメッセージ*>
- 発言者「<*発言したユーザ名*>」内容「<*発言内容* (複数行)>」
- 発言者「<*発言したユーザ名*>」内容「<*発言内容* (複数行)>」
- 発言者「<*発言したユーザ名*>」内容「<*発言内容* (複数行)>」
（続く）
」
...
4. その結論について、私は何かアクションを取る必要があるか？もしあれば、何をすればいいか？(*アクション*)
    4-1. 質問者が取りうる行動とそれぞれのメリット・デメリット・実現可能性をそれぞれ一行以上考え、結論としてアクションとしてまとめてください
...
注意点として、最初に与えられたユーザの要望を必ず満たすように要約文を出力してください。
"""
